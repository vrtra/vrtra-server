...
battlefield_object.onEventFinish = function(player, csid, option)
    ...
    if csid == 32001 then
        if player:getCurrentMission(COP) == xi.mission.id.cop.THE_MOTHERCRYSTALS then
            if  player:hasKeyItem(xi.ki.LIGHT_OF_MEA) and player:hasKeyItem(xi.ki.LIGHT_OF_HOLLA) then
                player:addExp(1500)
                player:addKeyItem(xi.ki.LIGHT_OF_DEM)
                player:messageSpecial(ID.text.CANT_REMEMBER,xi.ki.LIGHT_OF_DEM)
            ...
            elseif not player:hasKeyItem(xi.ki.LIGHT_OF_DEM) then
                player:setVar("cspromy3",1)
                player:addKeyItem(xi.ki.LIGHT_OF_DEM)
                player:addExp(1500)
                player:messageSpecial(ID.text.CANT_REMEMBER,xi.ki.LIGHT_OF_DEM)
                player:addStatusEffectEx(xi.effect.TELEPORT,0,xi.teleport.id.EXITPROMDEM,0,1)
            end
